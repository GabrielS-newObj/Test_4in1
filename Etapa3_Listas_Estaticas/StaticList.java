...
import Etapa3_Listas_Estaticas.ExceptionsFromList.OverflowException;
import Etapa3_Listas_Estaticas.ExceptionsFromList.UnderflowException;
...
public class StaticList<E> implements List<E> {
    ...
    public int contaElementos(E el) throws NullPointerException{
        if (el == null) throw new NullPointerException("Lista vazia!");

        return contaElementos(el, 0,0);
    }

    private int contaElementos(E el, int index, int times){
        if (index == elements.length) return times;

        if (el.equals(elements[index])) times++;

        return contaElementos(el, index+1, times);
    }
